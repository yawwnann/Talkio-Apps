...
class _TherapistPatientDetailPageState
  ...
  Future<void> _fetchPatientData() async {
    ...
    try {
      ...
      final artikulasiFuture = _apiService.getArtikulasiSummary(widget.patientId);
      ...
      if (results[4].statusCode == 200) {
        ...
        if (artikulasiData is Map<String, dynamic> && artikulasiData['status'] == 'success') {
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionCard(
              ...
              trailing: _exercises.isNotEmpty
                  ? Container(
                  ...
                  : null,
              child: _buildExerciseHistory(),
              isEmpty: _exercises.isEmpty,
              emptyMessage: 'Belum ada riwayat latihan.',
            ),
            const SizedBox(height: 16),
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: isEmpty
                ? Text(emptyMessage, style: GoogleFonts.poppins(fontSize: 13, color: const Color(0xFF94A3B8)))
                : child,
          ),
        ],
      ),
  ...
  Widget _buildProfileCard() {
    ...
    final diagnosisLabel = diagnoses.isNotEmpty ? diagnoses.map((d) => d['label'] ?? d['result'] ?? '').join(', ') : 'Belum ada';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [AppConstants.primaryBlue, Color(0xFF0D47A1)]),
        borderRadius: BorderRadius.circular(20),
        boxShadow: [BoxShadow(color: AppConstants.primaryBlue.withValues(alpha: 0.3), blurRadius: 12, offset: const Offset(0, 4))],
      ),
      child: Column(
        children: [
          Row(
            children: [
              Container(
                ...
                child: Center(child: Text(patientName.isNotEmpty ? patientName[0].toUpperCase() : '?', style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white))),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(patientName, style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                    const SizedBox(height: 4),
                    Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(8)), child: Text('AKTIF', style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.white, letterSpacing: 0.5))),
                  ],
                ),
              ),
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text('${sessions.length}', style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white)),
                Text('Sesi', style: GoogleFonts.poppins(fontSize: 12, color: Colors.white70)),
              ]),
            ],
          ),
          const SizedBox(height: 16),
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Flexible(child: Text(diagnosisLabel, style: GoogleFonts.poppins(fontSize: 12, color: Colors.white, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
            ]),
          ),
        ],
      ),
  ...
  Widget _buildQuickStats() {
    final sessions = _patientDetail!['therapySessions'] as List? ?? [];
    final lastSession = sessions.isNotEmpty ? _getLastSessionDate(sessions) : null;
    final dateOfBirthStr = _patientDetail!['dateOfBirth'];
    final age = dateOfBirthStr != null ? _calculateAge(DateTime.parse(dateOfBirthStr)) : '-';

    return Row(
      children: [
        Expanded(child: _buildStatItem('Usia', age, Icons.cake_outlined)),
        const SizedBox(width: 12),
        Expanded(child: _buildStatItem('Sesi Terakhir', lastSession ?? '-', Icons.calendar_today_outlined)),
        const SizedBox(width: 12),
        Expanded(child: _buildStatItem('Progress', '${_progressUploads.length}', Icons.trending_up_outlined)),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(value, style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B)), overflow: TextOverflow.ellipsis),
  ...
  String _getLastSessionDate(List sessions) {
    ...
    final sorted = List<Map<String, dynamic>>.from(sessions)..sort((a, b) {
  ...
  Widget _buildSectionTitle(String title) {
    return Text(title, style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF1E293B)));
  }
  ...
  Widget _buildProgressUploads() {
      ...
      return Center(
        child: Column(
          children: [
            ...
            Text('Belum ada progress', style: GoogleFonts.poppins(fontSize: 13, color: const Color(0xFF94A3B8))),
          ],
        ),
      );
    }
    ...
    return Column(
      children: _progressUploads.take(3).map((upload) {
        final IconData icon = upload.isVideo ? Icons.videocam : (upload.isAudio ? Icons.mic : Icons.image);
        final Color iconColor = upload.isVideo ? const Color(0xFF3B82F6) : (upload.isAudio ? const Color(0xFF8B5CF6) : const Color(0xFF16A34A));
        ...
        return Container(
          ...
          child: Row(
            children: [
              Container(width: 44, height: 44, decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: iconColor, size: 22)),
              const SizedBox(width: 12),
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(upload.formattedDateTime, style: GoogleFonts.poppins(fontSize: 11, color: const Color(0xFF94A3B8), fontWeight: FontWeight.w500)),
                  const SizedBox(height: 2),
                  Text(upload.parentNotes ?? 'Tanpa catatan', style: GoogleFonts.poppins(fontSize: 13, color: const Color(0xFF1E293B)), maxLines: 1, overflow: TextOverflow.ellipsis),
                  if (upload.duration != null) Text('Durasi: ${upload.durationLabel}', style: GoogleFonts.poppins(fontSize: 10, color: iconColor)),
                ]),
              ),
              IconButton(icon: const Icon(Icons.play_circle_outline, size: 22), color: iconColor, onPressed: () => _openMediaPreview(upload)),
            ],
          ),
        );
      }).toList(),
  ...
  Widget _buildExerciseHistory() {
    if (_exercises.isEmpty) {
      return Center(
        child: Column(
          children: [
            Icon(Icons.sports_esports_outlined, size: 40, color: Colors.grey[300]),
            const SizedBox(height: 8),
            Text('Belum ada riwayat', style: GoogleFonts.poppins(fontSize: 13, color: const Color(0xFF94A3B8))),
          ],
        ),
      );
    }
    ...
    return Column(
      children: _exercises.take(3).map((e) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              Container(width: 36, height: 36, decoration: BoxDecoration(color: AppConstants.primaryBlue.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.sports_esports, color: AppConstants.primaryBlue, size: 18)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(gameName, style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B)), maxLines: 1, overflow: TextOverflow.ellipsis),
                Text(dateStr, style: GoogleFonts.poppins(fontSize: 11, color: const Color(0xFF94A3B8))),
              ])),
              if (e['score'] != null) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFDCFCE7), borderRadius: BorderRadius.circular(6)), child: Text('${e['score']}', style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w600, color: const Color(0xFF16A34A)))),
            ],
          ),
        );
      }).toList(),
  ...
  Widget _buildCatatanLaporanSection() {
    ...
    return _buildSectionCard(
      ...
      trailing: IconButton(
        ...
        onPressed: () => context.push('/therapist/laporan/add', extra: widget.patientId),
      ),
    ...
    return Column(
      children: [
            ...
            child: Center(
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text('Belum ada catatan atau laporan', style: GoogleFonts.poppins(fontSize: 13, color: const Color(0xFF94A3B8))),
                ],
              ),
            ),
          )
        ...
        Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE2E8F0))),
          child: TextField(controller: _noteController, maxLines: 3, minLines: 2, decoration: InputDecoration(hintText: 'Tambah catatan...', hintStyle: GoogleFonts.poppins(fontSize: 13, color: const Color(0xFF94A3B8)), border: InputBorder.none)),
        ),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: AppConstants.primaryBlue, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
            child: Text('Simpan Catatan', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
          ),
        ),
        const SizedBox(height: 12),
        Row(
          children: [
            ...
            Padding(
              ...
              child: Text('atau', style: GoogleFonts.poppins(fontSize: 12, color: const Color(0xFF94A3B8))),
            ),
            const Expanded(child: Divider(color: Color(0xFFE2E8F0))),
          ],
        ),
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            ...
            label: Text('Buat Laporan Formal', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600)),
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.note_alt_outlined, color: Color(0xFF10B981), size: 18),
            ),
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(entry['content'] ?? '', style: GoogleFonts.poppins(fontSize: 13, color: const Color(0xFF334155)), maxLines: 2, overflow: TextOverflow.ellipsis),
                  const SizedBox(height: 4),
                  Text(_formatDate(entry['date']), style: GoogleFonts.poppins(fontSize: 10, color: const Color(0xFF94A3B8))),
                ],
              ),
            ),
          ],
      ...
      child: Row(
        children: [
          Container(
            width: 40,
            height: 40,
            decoration: BoxDecoration(color: const Color(0xFFFEE2E2).withValues(alpha: 0.5), borderRadius: BorderRadius.circular(8)),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(entry['title'] ?? 'Laporan', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B)), maxLines: 1, overflow: TextOverflow.ellipsis),
                if (status.isNotEmpty) ...[
                  const SizedBox(height: 2),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 1),
                    decoration: BoxDecoration(
                      color: status == 'SENT' ? const Color(0xFFD1FAE5) : const Color(0xFFFEF3C7),
                      borderRadius: BorderRadius.circular(4),
                    ),
                    child: Text(
                      status == 'SENT' ? 'Terkirim' : 'Draft',
                      style: GoogleFonts.poppins(fontSize: 9, fontWeight: FontWeight.w500, color: status == 'SENT' ? const Color(0xFF065F46) : const Color(0xFF92400E)),
                    ),
                  ),
                ],
              ],
  ...
  Widget _buildArtikulasiSection() {
    ...
    final recommendation = _artikulasiData?['evaluation']?['recommendation'] as String?;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Container(width: 36, height: 36, decoration: BoxDecoration(color: const Color(0xFF10B981).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.mic, color: Color(0xFF10B981), size: 20)),
            const SizedBox(width: 12),
            Expanded(child: Text('Progress Artikulasi', style: GoogleFonts.poppins(fontSize: 15, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B)))),
            if (totalPractice > 0) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: _needsReview > 0 ? const Color(0xFFFEE2E2) : const Color(0xFFF1F5F9), borderRadius: BorderRadius.circular(8)), child: Text(_needsReview > 0 ? '$totalPractice sesi • $_needsReview review' : '$totalPractice sesi', style: GoogleFonts.poppins(fontSize: 11, fontWeight: _needsReview > 0 ? FontWeight.w600 : FontWeight.normal, color: _needsReview > 0 ? const Color(0xFFDC2626) : const Color(0xFF64748B)))),
          ]),
          const SizedBox(height: 16),
          // Sound cards
          Row(children: ['R', 'S', 'L', 'N'].map((sound) => Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: _buildSoundCardCompact(sound, soundStats[sound] as Map<String, dynamic>?)))).toList()),
          if (recommendation != null && recommendation.isNotEmpty) ...[
            const SizedBox(height: 12),
            Container(width: double.infinity, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFFDBEAFE), borderRadius: BorderRadius.circular(10)), child: Row(children: [
              const Icon(Icons.lightbulb_outline, color: Color(0xFF2563EB), size: 16),
              const SizedBox(width: 8),
              Expanded(child: Text(recommendation, style: GoogleFonts.poppins(fontSize: 12, color: const Color(0xFF2563EB)))),
            ])),
          ],
        ],
      ),
  ...
  Widget _buildSoundCardCompact(String sound, Map<String, dynamic>? stat) {
    ...
    switch (status) {
      case 'mastered': bgColor = const Color(0xFFDCFCE7); textColor = const Color(0xFF16A34A); break;
      case 'improving': case 'practicing': bgColor = const Color(0xFFDBEAFE); textColor = const Color(0xFF2563EB); break;
      case 'struggling': bgColor = const Color(0xFFFEE2E2); textColor = const Color(0xFFDC2626); break;
      default: bgColor = const Color(0xFFF1F5F9); textColor = const Color(0xFF94A3B8);
    }
    ...
    return Container(
      ...
      child: Column(children: [
        Text(sound, style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: textColor)),
        const SizedBox(height: 4),
        Text('$correct/$total', style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.w600, color: textColor)),
        Container(margin: const EdgeInsets.only(top: 4), padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1), decoration: BoxDecoration(color: textColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(4)), child: Text(status == 'not_started' ? 'Belum' : (status == 'mastered' ? 'Oke' : 'Latihan'), style: GoogleFonts.poppins(fontSize: 8, fontWeight: FontWeight.w600, color: textColor))),
      ]),
    );
  }
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          child: Row(
                            children: [
                              Container(
                                ...
                                child: IconButton(
                                  icon: const Icon(Icons.play_arrow, color: AppConstants.primaryBlue),
                                  onPressed: () {
                                    final baseUrl = ApiService().dio.options.baseUrl.replaceAll('/api', '');
                                    final fullUrl = audioUrl.startsWith('http') ? audioUrl : '$baseUrl$audioUrl';
                        ...
                        children: [
                          Expanded(
                            child: GestureDetector(
                              onTap: () => setModalState(() => therapistRating = 'OKE'),
                              child: Container(
                                padding: const EdgeInsets.all(16),
                                decoration: BoxDecoration(
                                ...
                                child: Column(
                                  children: [
                                    Icon(
                                      Icons.check_circle,
                                      color: therapistRating == 'OKE' ? Colors.white : const Color(0xFF16A34A),
                                      size: 32,
                                    ),
                                    const SizedBox(height: 8),
                                    Text(
                                    ...
                                    Text(
                                      'Pengucapan baik',
                                      style: GoogleFonts.poppins(
                                        color: therapistRating == 'OKE' ? Colors.white70 : const Color(0xFF64748B),
                                        fontSize: 10,
                                      ),
                                    ),
                                  ],
                          ...
                          Expanded(
                            child: GestureDetector(
                              onTap: () => setModalState(() => therapistRating = 'BELUM_OK'),
                              child: Container(
                                padding: const EdgeInsets.all(16),
                                decoration: BoxDecoration(
                                ...
                                child: Column(
                                  children: [
                                    Icon(
                                      Icons.refresh,
                                      color: therapistRating == 'BELUM_OK' ? Colors.white : const Color(0xFFDC2626),
                                      size: 32,
                                    ),
                                    const SizedBox(height: 8),
                                    Text(
                                    ...
                                    Text(
                                      'Perlu latihan lagi',
                                      style: GoogleFonts.poppins(
                                        color: therapistRating == 'BELUM_OK' ? Colors.white70 : const Color(0xFF64748B),
                                        fontSize: 10,
                                      ),
                                    ),
                                  ],
                      ...
                      Slider(
                        ...
                        onChanged: (value) => setModalState(() => therapistScore = value.toInt()),
                      ),
                      const SizedBox(height: 24),
                      ...
                      if (suggestedWords.isNotEmpty) ...[
                        ...
                        Wrap(
                          spacing: 8,
                          runSpacing: 8,
                          children: suggestedWords.map((w) => Chip(
                            label: Text(w),
